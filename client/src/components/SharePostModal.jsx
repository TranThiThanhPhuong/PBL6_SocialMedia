...
import { X, Link, Facebook, Twitter, Linkedin, MessageCircle, ArrowRight, UserPlus, Globe, Zap, BookUser, ArrowUpRight } from "lucide-react";
  ...
  const handleShareToExternal = (platform) => {
    ...
    switch (platform) {
      ...
      case "zalo":
        ...
        url = `https://zalo.me/share?url=${encodeURIComponent(postUrl)}&text=${encodeURIComponent(text)}`;
        break;
      default:
        break;
    }
    ...
    <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50 overflow-y-auto py-8">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-lg flex flex-col">
        ...
        <div className="flex justify-between items-center px-5 py-3 border-b border-gray-100 relative">
          <h2 className="text-lg font-semibold flex-grow text-center">Chia sẻ</h2>
          <button onClick={onClose} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-black transition">
            <X size={22} />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-gray-900">{currentUser.full_name}</span>
        ...
        <div className="p-4 border-b border-gray-100">
          ...
          <div className="flex overflow-x-auto gap-4 no-scrollbar">
            {connections.map(friend => (
              <div key={friend._id} className="flex-shrink-0 w-20 text-center cursor-pointer">
                <img src={friend.profile_picture} className="w-16 h-16 rounded-full mx-auto" />
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-4">
            <button onClick={() => toast.success("Đã chia sẻ lên Story")} className="flex flex-col items-center gap-2">
            ...
            <button onClick={() => handleShareToExternal("facebook")} className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center text-white">
                <Facebook size={24} />
              </div>
              <p className="text-xs text-gray-700">Facebook</p>
            </button>
            <button onClick={() => handleShareToExternal("twitter")} className="flex flex-col items-center gap-2">
            ...
            <button onClick={() => handleShareToExternal("linkedin")} className="flex flex-col items-center gap-2">
            ...
            <button onClick={() => handleShareToExternal("zalo")} className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-full bg-blue-400 flex items-center justify-center text-white">
                <svg viewBox="0 0 100 100" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6">
